...
class PIC_2D:
    def __init__(self, params):
        ...
        self.particle_weight = 1 / (self.n_particles /
                                    (self.cells[0] * self.cells[1])
                                    )  # density/particles per cell
    ...
    def init_v_maxwellian(self):
        ...
        for i in range(self.n_particles):
            ...
            self.electron_vx[i] = np.sqrt(-np.log(r1x)) * np.cos(
                2 * np.pi * r2x)
            self.ion_vx[i] = 0

            self.electron_vy[i] = np.sqrt(-np.log(r1y)) * np.cos(
                2 * np.pi * r2y)
            self.ion_vy[i] = 0
    ...
    def init_v_two_stream(self):
        ...
        for i in range(self.n_particles):
            if self.electron_y[i] < stream_y_stop and self.electron_y[
                    i] > stream_y_start:
                r = np.random.rand()
    ...
    def cell_neighbors(self, x, dim):
        ...
        cells = self.cells[
            plat.DIM_MAP[dim]]  # number of cells along this dimension
    ...
    def update_n(self, particle_type):
        ...
        if particle_type == "electron":
            ...
            raise ValueError(
                "Unrecognized particle type: ".format(particle_type))
        ...
        for i in range(self.n_particles):
            ...
            area_upper_left = plat.math_utils.points_to_area((x_n, y_n),
                                                             (x0, y0))
            area_upper_right = plat.math_utils.points_to_area((x_n, y_n),
                                                              (x1, y0))
            area_lower_left = plat.math_utils.points_to_area((x_n, y_n),
                                                             (x0, y1))
            area_lower_right = plat.math_utils.points_to_area((x_n, y_n),
                                                              (x1, y1))
    ...
    def update_e(self):
        ...
        for i in range(self.nodes[0]):
            for j in range(self.nodes[1]):
                ...
                # E = -(phi_i - phi_i-1)/dx
                self.ex[i][j] = -(right_potential -
                                  left_potential) / self.dx[1]
    ...
    def update_v(self):
        ...
        for i in range(self.n_particles):
            ...
            e_10 = [
                self.ex[node_down][node_left], self.ey[node_down][node_left]
            ]
            e_11 = [
                self.ex[node_down][node_right], self.ey[node_down][node_right]
            ]
            ...
            e_particle = np.sum(list(map(lambda a, b: a * b, weights,
                                         e_nodes)),
                                axis=0)
    ...
    def calc_electrostatic_energy(self):
        ...
        for i in range(self.cells[0]):
            for j in range(self.cells[1]):
                ex_cell = np.mean([
                    self.ex[i][j], self.ex[i][j + 1], self.ex[i + 1][j],
                    self.ex[i + 1][j + 1]
                ])

                ey_cell = np.mean([
                    self.ey[i][j], self.ey[i][j + 1], self.ey[i + 1][j],
                    self.ey[i + 1][j + 1]
                ])
